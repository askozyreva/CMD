...
demo <- df %>% dplyr::select(id, gender, age, education, ethnicity, state,  party, party_enhanced, ideology) %>% 
  ...
  mutate(party_enhanced = factor(
    ...
    levels = c("Democrat", 
               ...
               "Republican"),
    labels = c("Democrats and democratic leaners",
                "Independent or not sure",
               "Republicans or republican leaners")
  )) %>% 
  dplyr::select(-age) %>% 
  pivot_longer(names_to = "measures", 
               values_to = "question",
               cols= c(gender, education, ethnicity, state, agegroup,  party, party_enhanced, ideology)) %>% 
  ...
  dplyr::select(-prop, -N) %>% 
  mutate(measures = factor(measures, levels = c("gender", "agegroup", "education", "state", "ethnicity",  "party", "party_enhanced", "ideology"),
                           labels = c("Gender", "Age group", "Education", "Region of residency", "Ethnicity", "Political party", "Political party detailed", "Political ideology"))) %>% 
  drop_na() %>% 
  arrange(factor(measures, levels = c("Gender", "Age group", "Education", "Region of residency", "Ethnicity", "Political party detailed","Political party", "Political ideology")))
...
table_demo <- demo_final %>%
  ...
  pack_rows("Political party identification, 7-point + not sure (%)", 28,35) %>%
  pack_rows("Political party grouped, 3-point (%)", 36,38) %>%
  pack_rows("Political ideology (%)", 39,42) 
...
table_demo_tex <- demo_final %>%
  kbl(booktabs = TRUE,
      caption = "Demographics \\label{tbl:demographics}",
      table.envir = "table*",
      col.names = c("", ""),
      format = "latex"
  ) %>%
  ...
  pack_rows("Region of residency (%)", 19,22)%>%
  pack_rows("Ethnicity (%)", 23,27) %>% 
  pack_rows("Political party, 7-point (%)", 28,35) %>%
  pack_rows("Political party (%)", 36,38) %>%
  pack_rows("Political ideology, 3-point (%)", 39,42)
table_demo_tex
...
```{r, table demo enchanced}

# add national benchmarks


demo_enhanced <- demo_final %>% 
  mutate(national = c(NA,   NA,   NA,   48.3,   51.7,    NA,   #gender
                      10.9,   18,  16.7,   16.2,   16.9,   21.2,  #age  
                      11.2,   27.3,   21.5,  28.2,   8.3,    3.3,   #education
                      38,   23.8,   17.3, 20.7,    #region
                      2.4,   63.2,   11.8,    16.4, 5.9,      #ethnicity
                     23,   12,   11,  12, 10, 11, 21, NA, # party 7-point ANES 2020 https://electionstudies.org/resources/anes-guide/top-tables/?id=21
                      46, 12, 42, # party 3-point ANES 2020 https://electionstudies.org/resources/anes-guide/top-tables/?id=22
                      17, 27,   22,   33 # ideology ANES 2020 https://electionstudies.org/resources/anes-guide/top-tables/?id=29
                      ))
...
```{r table demographics, include=TRUE}
table_demo_enhanced <- demo_enhanced %>%
  kbl(col.names = c("", "study sample", "benchmark")) %>%
  kable_paper("striped", full_width = F, position = "left") %>%
  column_spec(2, bold = T, color = "black", background = "#e5e5e5", width = "10em") %>%
  column_spec(1, bold = F, color = "black", width = "40em") %>%
  pack_rows("Sample size", 1,1) %>%
  ...
  pack_rows("Region of residency (%)", 19,22)%>%
  pack_rows("Ethnicity (%)", 23,27) %>% 
  pack_rows("Political party, 7-point and not sure (%)", 28,35) %>%
  pack_rows("Political party, 3-point (%)", 36,38) %>%
  pack_rows("Political ideology (%)", 39,42)
  save_kable(file = here("figs/table_demo_enhanced.png"), zoom = 1.5)
table_demo_enhanced


table_demo_enhanced_tex <- demo_enhanced %>%
  kbl(booktabs = TRUE,
      caption = "Demographics with benchmarks \\label{tbl:demographics}",
      table.envir = "table*",
      col.names = c("", "study sample", "benchmark"),
  ...
  pack_rows("Region of residency (%)", 19,22)%>%
  pack_rows("Ethnicity (%)", 23,27) %>% 
  pack_rows("Political party, 7-point scale (%)", 28,35) %>%
  pack_rows("Political party (%)", 36,38) %>%
  pack_rows("Political ideology, 3-point scale (%)", 39,42)
table_demo_enhanced_tex



write_lines(table_demo_enhanced_tex, here("figs/tbl_demo_enhanced.tex"))
...
speech_disinfo_p <- ggplot() +
  ...
  labs(title = "B. Free speech vs. disinformation\nby respondents' party affiliation")
speech_disinfo_p
...
ggsave(
  filename = here("figs/SI_freespeech_disinfo.png"),
...
ggsave(
  filename = here("figs/SI_freespeech_disinfo.pdf"),
  plot = speech_disinfo_plot_1,
  dpi = 300,
  units = 'cm',
  height = 10,
...
speech_disinfo_post_plot <- ggplot() +
  ...
  labs(title = "C. Free speech vs. disinformation\n(after the main task)")
...
speech_disinfo_p_post_plot <- ggplot() +
  ...
  labs(title = "D. Free speech vs. disinformation by party\n(after the main task)")
...
ggsave(
  filename = here("figs/SI_free_preppost.png"),
...
ggsave(
  filename = here("figs/SI_free_preppost.pdf"),
  plot = speech_disinfo_2,
  dpi = 300,
  units = 'cm',
  height = 12,
...
ggsave(
  filename = here("figs/SI_plot_accuracy.png"),
  plot = plot_acc,
  dpi = 300,
  units = 'cm',
  height = 20,
...
ggsave(
  filename = here("figs/SI_plot_accuracy.pdf"),
  plot = plot_acc,
  dpi = 300,
  units = 'cm',
  height = 20,
...
ggsave(
  filename = here("figs/SI_plot_harms.png"),
  plot = harms,
  dpi = 300,
  units = 'cm',
  height = 20,
...
ggsave(
  filename = here("figs/SI_plot_harms.pdf"),
  plot = harms,
  dpi = 300,
  units = 'cm',
  height = 20,
...
ggsave(
  filename = here("figs/SI_plot_outcomes.png"),
  plot = plot_outcomes,
  dpi = 300,
  units = 'cm',
  height = 14,
...
ggsave(
  filename = here("figs/SI_plot_outcomes.pdf"),
  plot = plot_outcomes,
  dpi = 300,
  units = 'cm',
  height = 14,
...
ggsave(
  filename = here("figs/SI_plot_freespeech.png"),
...
ggsave(
  filename = here("figs/SI_plot_freespeech.pdf"),
  plot = plot_freespeech,
  dpi = 300,
  units = 'cm',
  height = 22,
...
```{r, plot free speech support dichotomized,  fig.width = 12, fig.height= 8, fig.cap = "Free speech attitudes", echo=TRUE}
...
plot_free_bi <- ggplot() +
  ...
  labs(title = "A. Attitudes toward freedom of expression and its limits (dichotomized rating)") +
...
plot_free_pbi <- ggplot() +
  ...
  scale_fill_manual(values = fig_free_palette, breaks = c("Agree", "Disagree")) +
  labs(title = "B. Attitudes toward freedom of expression and its limits by party affiliation (dichotomized rating)") +
  labs(subtitle = "", y = "% of respondents", x =
         "")
plot_free_pbi
...
ggsave(
  filename = here("figs/SI_plot_freespeech_bi.png"),
...
ggsave(
  filename = here("figs/SI_plot_freespeech_bi.pdf"),
  plot = plot_freespeech_bi,
  dpi = 300,
  units = 'cm',
  height = 20,
...
ggsave(
  filename = here("figs/SI_er_ecdf_plot.png"),
  plot = er_ecdf_plot,
  dpi = 300,
  units = 'cm',
  height = 15,
...
ggsave(
  filename = here("figs/SI_er_ecdf_plot.pdf"),
  plot = er_ecdf_plot,
  dpi = 300,
  units = 'cm',
  height = 14,
